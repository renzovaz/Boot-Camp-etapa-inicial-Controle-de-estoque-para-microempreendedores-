...
@click.group()
@click.option(
    "--data", type=click.Path(path_type=Path), help="Caminho alternativo para o arquivo de dados"
)
@click.pass_context
def cli(ctx, data):
...
def add(obj, name, quantity, min_stock, price):
    ...
    try:
        product = inv.add_product(
            name=name, quantity=quantity, min_stock=min_stock, unit_price=price
        )
        storage.save(inv)
        _ok(
            f"Produto '{product.name}' salvo — estoque: {product.quantity} unidades @ R$ {product.unit_price:.2f}"
        )
...
def sell(obj, name, quantity):
    ...
    try:
        ...
        _ok(
            f"Venda registrada: {quantity}× '{product.name}' = R$ {total:.2f}. Saldo: {product.quantity} unidades."
        )
        if product.quantity <= product.min_stock:
...
def list_products(obj):
    ...
    for p in products:
        status = (
            click.style("BAIXO", fg="red")
            if p.quantity <= p.min_stock
            else click.style("OK", fg="green")
        )
        click.echo(
            fmt.format(p.name[:28], p.quantity, p.min_stock, f"R$ {p.unit_price:.2f}", status)
        )
